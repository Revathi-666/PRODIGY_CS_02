...
def encrypt_image(image_path, key):
    ...
    for x in range(width):
        for y in range(height):
            ...
            encrypted_pixel = xor_encrypt_decrypt(pixel, key)
...
def decrypt_image(encrypted_image_path, key):
    ...
    for x in range(width):
        for y in range(height):
            pixel = img.getpixel((x, y))
            decrypted_pixel = xor_encrypt_decrypt(pixel, key)
...
def select_image():
    ...
    root = Tk()
    root.withdraw()
    file_path = filedialog.askopenfilename(title="Select Image File", filetypes=(("PNG files", "*.png"), ("JPEG files", "*.jpg;*.jpeg"), ("All files", "*.*")))
    root.destroy()
    if not file_path:
